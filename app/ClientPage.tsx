...
function ClientContent() {
  ...
  const { playConfetti, setPlayConfetti, showGoalReachedModal, setShowGoalReachedModal, settings, toggleTimer } =
    useTimer()
      ...
      <div className="flex items-center justify-center min-h-screen">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-xs text-muted-foreground max-w-xs text-center">
            This may take a moment. We've disabled automatic sync to prevent freezing.
          </p>
        </div>
      </div>
    )
  ...
  return (
    <>
      <main className="container flex flex-col items-center justify-start min-h-screen pt-0 mx-auto overflow-hidden">
        <Tabs value={activeTab} onValueChange={setActiveTab} defaultValue="timer" className="w-full max-w-7xl">
          <div className="w-full flex items-center justify-between p-2">
            ...
            <div className="flex items-center space-x-2">
              <TabsList className="h-8 bg-transparent">
                <TabsTrigger value="journal" className="bg-transparent px-2" aria-label="View journal">
                  <BookOpen className="h-3.5 w-3.5" />
                  <span className="sr-only">Journal</span>
                </TabsTrigger>
                <TabsTrigger value="history" className="bg-transparent px-2" aria-label="View history">
                  <History className="h-3.5 w-3.5" />
                  <span className="sr-only">History</span>
                </TabsTrigger>
                <TabsTrigger value="settings" className="bg-transparent px-2" aria-label="View settings">
          ...
          <TabsContent value="timer" className="w-full h-[calc(100vh-60px)] overflow-hidden">
            <Timer useIconButtons={true} />
          </TabsContent>
          <TabsContent value="journal" className="w-full h-[calc(100vh-60px)] overflow-hidden">
            <Suspense fallback={<JournalFallback />}>{activeTab === "journal" && <JournalPage />}</Suspense>
          </TabsContent>
          <TabsContent value="history" className="w-full h-[calc(100vh-60px)] overflow-auto">
            <Suspense fallback={<HistoryFallback />}>{activeTab === "history" && <HistoryPage />}</Suspense>
          </TabsContent>
          <TabsContent value="settings" className="w-full h-[calc(100vh-60px)] overflow-auto">
            <Suspense fallback={<SettingsFallback />}>{activeTab === "settings" && <SettingsPage />}</Suspense>
          </TabsContent>
        </Tabs>
      </main>

      {/* Confetti animation that plays 3 times when a pomodoro is completed */}
      <ConfettiAnimation play={playConfetti} times={3} onComplete={() => setPlayConfetti(false)} />
