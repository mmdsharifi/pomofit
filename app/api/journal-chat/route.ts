...
export async function POST(request: NextRequest) {
  try {
    ...
    const systemPrompt = `You are a helpful journal AI assistant. You have access to the user's journal entries and can:
...
Be empathetic, insightful, and helpful. Keep responses concise but meaningful.`
    ...
    if (errorMessage.includes("decommissioned") || errorMessage.includes("not supported")) {
      return NextResponse.json({
        success: false,
