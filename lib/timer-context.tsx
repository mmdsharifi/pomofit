...
interface TimerContextType {
  ...
  handleNoteSubmit: (note: string, tags: string[], autoStartRest?: boolean) => void
  settings: {
    pomodoroTime: number
...
const countTodaysPomodoroSessions = (): number => {
  try {
    ...
    return todaySessions.filter((session) => session.mode === "pomodoro").length
  } catch (error) {
...
function TimerProviderWithTasks({ children }: { children: ReactNode }) {
  ...
  return <TimerProviderInner taskContext={taskContext}>{children}</TimerProviderInner>
}
...
function TimerProviderInner({
  ...
  const [notificationsEnabled, setNotificationsEnabled] = useLocalStorage("pomofit-notifications-enabled", false)
  ...
  const currentTaskRef = useRef<{ id: string | null; title: string | null }>({ id: null, title: null })
    ...
    if (isRunning && timeLeft === 0) {
      ...
      if (mode === "pomodoro") {
        ...
        setTimeout(() => {
          ...
          const nextBreakMode = newPomodorosCompleted % 4 === 0 ? "longBreak" : "shortBreak"
          nextModeRef.current = nextBreakMode

          // Show toast notification
          toast({
            title: nextBreakMode === "longBreak" ? "Time for a long break! 🌴" : "Pomodoro completed! ✅",
            description:
  ...
  const handleNoteSubmit = (note: string, tags: string[], autoStartRest = true) => {
    console.log("Adding session with task:", currentSessionRef.current.taskId, currentSessionRef.current.taskTitle)
    ...
    const session = {
      ...
      note,
      tags,
      taskId: currentSessionRef.current.taskId,
      taskTitle: currentSessionRef.current.taskTitle,
    }
  ...
  const getRandomMotivationalMessage = useCallback(() => {
    const randomIndex = Math.floor(Math.random() * restMotivationalMessages.length)
