...
import { CalendarIcon, BarChart2, ChevronLeft, ChevronRight } from "lucide-react"
import { Calendar } from "@/components/ui/calendar"
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover"
import { Button } from "@/components/ui/button"
import { Tabs, TabsList, TabsTrigger } from "@/components/ui/tabs"
import { Card, CardContent } from "@/components/ui/card"
import { getHistoryByDate, getTaskCompletionsByDate } from "@/lib/history-utils"
...
export default function HistoryClient() {
  ...
  const sessions = useMemo(() => {
    if (!isClient) return []
    try {
      console.log("HistoryClient: Getting sessions for date:", selectedDate)
      const result = getHistoryByDate(selectedDate)
      console.log("HistoryClient: Retrieved sessions:", result.length, result)
      return result
    } catch (error) {
  ...
  const completions = useMemo(() => {
    if (!isClient) return []
    try {
      console.log("HistoryClient: Getting completions for date:", selectedDate)
      const result = getTaskCompletionsByDate(selectedDate)
      console.log("HistoryClient: Retrieved completions:", result.length, result)
      return result
    } catch (error) {
  ...
  // Debug logging when sessions or completions change
  useEffect(() => {
    console.log("HistoryClient: Sessions updated:", sessions.length)
    console.log("HistoryClient: Completions updated:", completions.length)
  ...
  const totalFocusTime = useMemo(() => {
    try {
      ...
      console.log("HistoryClient: Calculated focus time:", focusTime)
      return focusTime
    } catch (error) {
  ...
  const totalPomodoros = useMemo(() => {
    try {
      ...
      console.log("HistoryClient: Calculated pomodoro count:", pomodoroCount)
  ...
  const totalCompletedTasks = useMemo(() => {
    try {
      ...
      console.log("HistoryClient: Calculated completed tasks:", taskCount)
      return taskCount
    } catch (error) {
            ...
            <div className="grid grid-cols-3 gap-2 w-full sm:w-auto">
              <Card className="border border-border">
                <CardContent className="p-2 text-center">
                  ...
                  <p className="text-lg font-bold">{formatDurationForDisplay(totalFocusTime)}</p>
                </CardContent>
              </Card>
              <Card className="border border-border">
                <CardContent className="p-2 text-center">
