...
import { Check, AlertTriangle, RefreshCw, Download, Upload, Info } from "lucide-react"
...
export default function SettingsClient() {
  ...
  const handleExportData = () => {
    try {
      ...
      a.href = url
      a.download = `pomofit-export-${new Date().toISOString().split("T")[0]}.csv`
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (!parsedData) {
          setImportError("Could not parse the import file. Please make sure it's a valid Pomofit export.")
          return
        }
  ...
  const handleImportConfirm = () => {
    try {
      ...
      if (parsedData.sessions && parsedData.sessions.length > 0) {
        localStorage.setItem("pomofit-history", JSON.stringify(parsedData.sessions))
      }

      // Import task completions
      if (parsedData.taskCompletions && parsedData.taskCompletions.length > 0) {
        localStorage.setItem("pomofit-task-completions", JSON.stringify(parsedData.taskCompletions))
      }
      ...
      toast({
        ...
        description: "Your data has been imported. The page will reload to apply changes.",
      })
    ...
    return (
      ...
      >
        {hasError ? (
          ...
          <Player
            ref={(el) => (playerRefs.current[workout.id] = el)}
            src={lottieUrl}
            autoplay={false}
            loop={true}
            renderer="canvas"
      ...
      <div className="grid gap-6">
            ...
            <div>
              <div className="inline-flex rounded-md border border-input overflow-hidden">
                <Button
                  variant="ghost"
                  size="sm"
                  className={`rounded-none border-0 ${theme === "light" ? "bg-accent" : ""}`}
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  className={`rounded-none border-0 ${theme === "dark" ? "bg-accent" : ""}`}
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  className={`rounded-none border-0 ${theme === "system" ? "bg-accent" : ""}`}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Supabase integration is temporarily disabled</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex flex-col gap-4">
              <p className="text-sm text-muted-foreground">
                The Supabase integration has been temporarily disabled. The app is currently working in offline mode
                only. All data is stored locally in your browser.
              </p>
          ...
          <CardContent className="space-y-6">
            <div className="space-y-2">
              <div className="flex justify-between">
                <label htmlFor="pomodoro-time">Pomodoro Duration: {pomodoroTime} minutes</label>
              </div>
              <Slider
                id="pomodoro-time"
                min={5}
            ...
            <div className="space-y-2">
              <div className="flex justify-between">
                <label htmlFor="short-break-time">Short Break: {shortBreakTime} minutes</label>
              </div>
              <Slider
                id="short-break-time"
                min={1}
            ...
            <div className="space-y-2">
              <div className="flex justify-between">
                <label htmlFor="long-break-time">Long Break: {longBreakTime} minutes</label>
              </div>
              <Slider
                id="long-break-time"
                min={5}
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">Set your daily goal (1-20 pomodoros)</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Select which workouts to show during breaks</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Export and import your Pomofit data</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <Alert>
              ...
              <AlertDescription>
                Your data is stored in your browser's local storage. Export your data regularly to avoid losing it when
              ...
              <Dialog open={importDialogOpen} onOpenChange={setImportDialogOpen}>
                ...
                <DialogContent className="sm:max-w-md">
                  <DialogHeader>
                    ...
                    <DialogDescription>Upload a Pomofit CSV export file to restore your data.</DialogDescription>
                  </DialogHeader>
                  ...
                  <div className="space-y-4 py-4">
                    <div className="flex flex-col gap-2">
                      ...
                      />
                      <p className="text-xs text-muted-foreground">Only Pomofit export files are supported</p>
                    </div>
                  ...
                  <DialogFooter>
                    ...
                    <Button onClick={handleImportConfirm} disabled={!importPreview}>
                      Import Data
                    </Button>
                  </DialogFooter>
                </DialogContent>
            ...
            <p className="text-xs text-muted-foreground">
              Importing data will replace your current data and reload the application.
            </p>
          </CardContent>
        </Card>

        <Button onClick={handleSave} className="w-full max-w-xs mx-auto" aria-label="Save all settings">
          Save Settings
        </Button>
      </div>
    </div>
