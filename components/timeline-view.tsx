...
export default function TimelineView({
  ...
  const [timeRange, setTimeRange] = useState<{ start: number; end: number }>({ start: 8, end: 20 })
  const containerRef = useRef<HTMLDivElement>(null)

  // Debug logging
  useEffect(() => {
    console.log("TimelineView received sessions:", sessions.length, sessions)
    console.log("TimelineView received completions:", completions.length, completions)
  ...
  const assignColumns = (events: ProcessedEvent[]) => {
    if (!events || events.length === 0) {
      console.log("No events to assign columns to")
      return { events: [], columnCount: 1 }
    }

    try {
      console.log("Assigning columns to", events.length, "events")
      ...
      sortedEvents.forEach((event, index) => {
        ...
        console.log(`Event ${index} assigned to column ${column}:`, event.title)
      })

      // Count the number of columns needed
      const columnCount = Math.max(1, Math.max(...sortedEvents.map((e) => e.column), 0) + 1)

      console.log("Column assignment complete. Total columns:", columnCount)
  ...
  const processedEvents = useMemo(() => {
    try {
      console.log("Processing events - sessions:", sessions.length, "completions:", completions.length)

      // Ensure sessions and completions are arrays
      const safeSessions = Array.isArray(sessions) ? sessions : []
      const safeCompletions = Array.isArray(completions) ? completions : []

      console.log("Safe sessions:", safeSessions.length)
      console.log("Safe completions:", safeCompletions.length)
        ...
        .map((session, index) => {
          try {
            ...
            if (isNaN(startTime.getTime())) {
              console.warn(`Invalid date in session at index ${index}:`, session)
              return null
            }

            const endTime = new Date(startTime.getTime() + (session.duration || 0) * 1000)
            const taskName =
            ...
            const event = {
              id: session.id || `session-${Math.random().toString(36).substring(2, 9)}`,
            ...
            console.log(`Processed session ${index}:`, event)
            return event
          } catch (error) {
      ...
      console.log("Processed session events:", sessionEvents.length)
      ...
      const completionEvents = safeCompletions
        .map((completion, index) => {
          try {
            ...
            if (isNaN(startTime.getTime())) {
              console.warn(`Invalid date in completion at index ${index}:`, completion)
              return null
            }
            ...
            const event = {
              id: completion.id || `completion-${Math.random().toString(36).substring(2, 9)}`,
            ...
            console.log(`Processed completion ${index}:`, event)
            return event
          } catch (error) {
            console.error(`Error processing completion at index ${index}:`, error, completion)
            return null
          }
        })
        .filter(Boolean) as ProcessedEvent[]

      console.log("Processed completion events:", completionEvents.length)
  ...
  const hourMarkers = useMemo(() => {
    try {
      return Array.from({ length: timeRange.end - timeRange.start + 1 }, (_, i) => timeRange.start + i).map((hour) => {
  ...
  console.log("Timeline render debug info:", debugInfo)
      ...
      <Card className={className}>
        <CardContent className="p-4">
          <div className="flex flex-col items-center justify-center h-[400px]">
            <p className="text-muted-foreground mb-4">No sessions found for this date</p>
            {process.env.NODE_ENV === "development" && (
              <div className="text-xs text-muted-foreground bg-muted p-4 rounded max-w-md">
                ...
                <p>
                  Time range: {debugInfo.timeRange.start}:00 - {debugInfo.timeRange.end}:00
                </p>
        ...
        <div className="relative" style={{ height: `${containerHeight}px` }} ref={containerRef}>
          ...
          <div className="absolute left-0 top-0 bottom-0 w-16 border-r border-border bg-background z-10">
            {hourMarkers.map((marker, index) => (
              ...
              >
                {marker.hour}
                <div className="absolute left-16 w-full border-t border-border" style={{ top: "50%" }}></div>
              </div>
            ))}
          </div>
