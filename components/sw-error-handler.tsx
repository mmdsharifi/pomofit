    ...
    const handleError = (event: ErrorEvent) => {
      if (event.filename && event.filename.includes("service-worker.js")) {
        ...
        toast({
          ...
          description: "Some offline features may not work. Please reload the page.",
          duration: 5000,
        })
      }
    }
  ...
  if (hasError) {
    return (
      <div className="fixed bottom-4 right-4 z-50 bg-background border rounded-lg shadow-lg p-4">
        ...
        <p className="text-sm text-muted-foreground mb-3">Some offline features may not work properly.</p>
        <button
